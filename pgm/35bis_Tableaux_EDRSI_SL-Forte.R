...
cat(texte_limitretraites, file=c("sorties/limitations_retraite_SL_Forte.csv"), sep="\n")

write_csv2(limitretraites_impute, "sorties/limitations_retraite_SL_Forte.csv", append=TRUE, col_names=TRUE)
...
limitretraites_impute %>% 
  filter(Sexe == "Hommes") %>% 
  select(AENQ, AGE, retraite_limite, retraite_non_limite) %>% 
  pivot_longer(c(retraite_non_limite, retraite_limite)) %>% 
  ggplot(aes(AGE, value, fill=rev(name))) +
  geom_area() +
  facet_grid(cols = vars(AENQ))

limitretraites_impute %>% 
  filter(Sexe == "Hommes") %>% 
  mutate(retraite = retraite_non_limite + retraite_limite) %>% 
  ggplot(aes(AGE, retraite, alpha=AENQ)) +
  geom_line()
